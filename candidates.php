...
if($_GET['filters'] == "true"){
  ...
  $counterSql =  "SELECT COUNT(DISTINCT candidate_id) AS id FROM candidate WHERE candidate_id != '' $qString ".$criterion;
    ...
    $counter = mysqli_query($dbc, $counterSql);
...
$counter = mysqli_query($dbc, "SELECT COUNT(DISTINCT candidate_id) AS id FROM candidate WHERE full_name != '' $criterion");
$num = mysqli_fetch_array($counter);
$leadCount = $num["id"];
if ($num["id"] == NULL){
  $leadCount = 0;
                                              ...
                                              while($candidate = mysqli_fetch_array($query)){
                                                ...
                                                for($x = 0; $x <= $countOfColumnPreference; $x++){
                                                  ...
                                                  if($string == "owner"){
                                                    ...
                                                    $rr = mysqli_query($dbc, $getUser);
                                                    $user = mysqli_fetch_array($rr);
